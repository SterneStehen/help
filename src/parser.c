...
void	raise_error(char *msg) { printf("%s\n", msg); }
...
void	init_commands(t_simple_cmds *tables) {
  while (tables != NULL) {
    tables->arguments = extract_cmd_args(tables->command, tables->token_arg);
    tables = tables->next;
  }
}
...
t_node	*add_quote(char *input_str, int *current_index, int *previous_index,
  ...
  if (*current_index - 1 != *previous_index)
    token = insert_token(
        token,
        duplicate_string_range(input_str, *previous_index, *current_index),
        STRING);
  *previous_index = *current_index;
  return (token);
}
...
t_node	*plus_redire(char *input_str, int *current_index, int *previous_index,
  ...
  redirection_type =
      determine_redirection_type(input_str, *previous_index, *current_index);
...
char **pipe_split(char *str, int start, int end, int index) {
    ...
    tokens = malloc(sizeof(char *) * (pipes_count + 2));  // Allocate space for tokens and NULL terminator
    if (tokens == NULL)
        return NULL;

    while (str[++end] != '\0') {
        end = pass_quoted_sect(str, end);
        if (str[end] == '|' && str[end - 1] != '\'' && calcul_escaped_charact(str, end) % 2 == 0) {
