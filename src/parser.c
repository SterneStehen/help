...
t_class	identify_redirection_type(char *str, int start, int end)
{
	...
	if (length == 2)
	{
		return ((start_char == '>') ? GREAT_GREAT : LESS_LESS);
	}
	else if (length == 1)
	{
		return ((start_char == '>') ? GREAT : LESS);
	}
	return (UNKNOWN);
}
...
void	manage_process_tokens(t_node *tokens)
{
	...
	while (tokens != NULL)
	{
		...
		temp = delite_quotes(tokens->data);
		free(tokens->data);
		tokens->data = temp;
		tokens = tokens->next;
	}
...
void	handle_command_table(t_simple_cmds *table, t_tools *tools)
{
	...
	{
		temp = delite_quotes(table->command);
		free(table->command);
		table->command = temp;
		optimize_commands(table);
	}
	if (are_strings_equal(table->command, "echo") == FALSE
		&& are_strings_equal(table->command, "export") == FALSE)
...
/* Checks if a character is a special ASCII character.
   Returns 1 if the character is a special ASCII character, otherwise returns 0.
 */
...
char	**allocate_cmd_args(char *cmd, t_node *token, int *size)
{
	...
	*size = (cmd == NULL) ? initial_size : initial_size + 1;
...
t_node	*split_to_tokens(char *str, t_node *token)
{
	...
	while (str[current_index] != '\0')
	{
		...
		token = plus_quote(str, &current_index, &previous_index, token);
		token = plus_string(str, &current_index, &previous_index, token);
...
t_node	*plus_quote(char *input_str, int *current_index, int *previous_index,
		t_node *token)
{
	char	quote_char;
	int		stop_search;
	...
	while (input_str[*current_index] != '\0' && stop_search == 0)
	{
		if (input_str[*current_index] == quote_char
			&& count_escaped_characters(input_str, *current_index) % 2 == 0)
			stop_search = 1;
		(*current_index)++;
	}
		...
		t_node *token_list)
{
	...
	if (input_str[*current_index] == '\0' || find_char_in_str(REDIRECTIONS,
		...
		t_node *token_list)
{
	...
	{
		if ((find_character_type(str[*idx]) & (IS_ALPHA | IS_DIGIT | IS_PRINTABLE)) == 0)
		{
			break ;
		}
		(*idx)++;
...
int	parse_shell_input(t_tools *shell)
{
	...
	{
		token = NULL;
		token = split_to_tokens(shell->pipes[i++], token);
	...
	return (audit_command(shell->commands) ? 1 : 0);
}
...
char	**split_pipe_segments(char *str, int start, int end, int index)
{
	...
	if (str == NULL)
		return (NULL);
	pipes_count = count_pipe_segment(str);
	if (!smalloc(sizeof(char *) * (pipes_count + 1)))
		return (NULL);
	tokens = malloc(sizeof(char *) * (pipes_count + 1));
	while (tokens == NULL)
	{
		if (!smalloc(sizeof(char *) * (pipes_count + 1)))
			return (NULL);
	...
	while (str[++end] != '\0')
	{
		end = skip_quoted_sections(str, end);
		if (strlen(str) > end && str[end] != '\0' && str[end] == '|' && str[end
			- 1] != SQUOTE && count_escaped_characters(str, end) % 2 == 0)
		{
...
int	skip_quoted_sections(char *str, int index)
{
	...
	if (str[index] != SQUOTE && str[index] != DQUOTE)
		return (index);
	quote = str[index++];
	while (str[index] != quote && count_escaped_characters(str, index) % 2 == 0)
	{
		if (str[index] == '\0')
			return (index);
		index++;
...
int	count_pipe_segment(char *str)
{
	...
	while (str[++i] != '\0')
	{
		i = skip_quoted_sections(str, i);
		if (ft_strlen(str) > i && str[i] != '\0' && str[i] == '|'
			&& count_escaped_characters(str, i) % 2 == 0)
			count++;
	}
	return (count + 1);
}
