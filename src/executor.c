 ...
 * This function verifies if there are any invalid redirections in the given command list.
 * It ensures that each redirection token is followed by a valid token and not another redirection. */
...
void run_heredoc_commands(t_simple_cmds *start, t_simple_cmds *last, t_tools *sh) {
    t_node *token;
    char *tmp_str;
    ...
    while (start != last->next) {
        ...
        while (token != NULL) {
            ...
            if (token->class == 'H') {
                ...
                start->hd_file_name = generate_temp_heredoc(start, token->next->data, sh);
            }
            token = token->next;
        }
        start = start->next;
 ...
 * If an invalid redirection is found, it processes heredoc commands and returns 1. */
 ...
 * This function runs each command in the pipeline, handling the necessary process forking and piping.
 ...
 * This function initializes a pipe and exits with an error message if the pipe creation fails. */
 ...
 * This function forks the current process and sets up the child process for command execution,
...
int create_child_process(t_tools *shell, int *fd) {
    ...
    if (pid <= -1) {
        ...
        shell->flag_log = 1; // Setting log flag
        close(fd[0]);
...
void execute_parent_process(t_simple_cmds *table, t_tools *shell, int *fd) {
    shell->flag_log = 0; // Reset log flag before executing commands
 ...
 * This function sets up the pipeline, forks the process, and manages redirections and command execution
 ...
 * This function checks the command's redirections list to determine if it contains any output redirection tokens. */
...
void spawn_and_execute(t_simple_cmds *table, t_tools *shell, int *has_redir) {
    ...
    shell->flag_log = 0; // Reset log flag after process completion
 ...
 * to their original values, ensuring that subsequent commands can execute correctly. */
 ...
 * This function manages the execution of commands, including signal handling, redirections, and command execution. */
int handle_execution(t_simple_cmds *cmd_table, t_tools *sh) {
    ...
    if (!invalid_redirection(sh, cmd_table->redirections, cmd_table)) {
        sh->flag_log = 1; // Setting log flag
 ...
 * This function handles the execution of a single command, managing forking, redirections, and cleanup. */
 ...
 * This function prints an error message and exits the program with the given status. */
void error_exit(t_tools *shell, const char *msg, int exit_st) {
    if (shell->flag_log == 1) {
        printf("%s: %s:  %s\n", "minishell: ", shell->message, msg);
        shell->flag_log = 0; // Reset log flag after printing the error
    }
    if (exit_st != 2)
...
void execute_final_command(char *command_path, t_simple_cmds *table, t_tools *shell) {
    char **cmd_args_copy;
    char **env_copy;
    int exec_result;
...
int finalize_command_path(char *command_path, t_simple_cmds *table, t_tools *shell) {
    int cmd_length;
 ...
 /* This function checks for specific exit conditions for a command and exits with appropriate status codes. */
...
int exit_if_null(t_tools *shell, char *cmd_path, t_simple_cmds *table) {
    ...
    int exit_code = handle_exit_conditions(shell, table, cmd_path);
    if (exit_code != -1) {
        return exit_code;
    }
 ...
 * including managing redirections, finding the command path, and executing built-in commands. */
 ...
 * This function executes the given command tables, managing pipelines and simple commands as needed. */
