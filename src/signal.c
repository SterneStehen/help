...
/*   Updated: 2024/07/07 17:54:42 by smoreron         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * This handler writes "^C\n" to the standard output and resets the readline buffer
 * to handle an interrupt signal (Ctrl+C) gracefully without exiting the program. */
 ...
 * This function configures the signal handler for SIGINT to use the given handler function,
 * allowing for custom behavior when the interrupt signal (Ctrl+C) is received. */
 ...
 * This function modifies the terminal settings to prevent the echoing of control characters,
 ...
 * Restores the terminal settings, ignores SIGQUIT, and sets the SIGINT handler to default.
 * This ensures that the child process behaves correctly with respect to terminal signals. */
 ...
 * This ensures that the parent process handles signals appropriately during its execution. */
