...
void handle_escaped_chars(char *str, int *index) {
    ...
    *index = i;
    int to_write = (count % 2 == 0) ? count / 2 : (count / 2) + 1;
...
int print_without_quotes(char *input) {
    ...
    while (input[idx] != '\0') {
        ...
        while ((input[idx] == ' ' && input[idx + 1] == ' ' && !found_quote) || input[idx] == '\\') {
            idx++;
        }
...
void process_args(char **arguments, int startIndex) {
    int *p_current_space = (int *)smalloc(sizeof(int));
    int *p_next_space = (int *)smalloc(sizeof(int));
    ...
    while (arguments[startIndex] != NULL) {
        ...
        if (arguments[startIndex + 1] != NULL && (!(*p_current_space) || (*p_next_space))) {
            write(1, " ", 1);
        }
        startIndex++;
    }
